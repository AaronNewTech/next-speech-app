...
const SpeechPractice: React.FC<EmailProps> = ({ email, setEmail }) => {
  const { user } = useAuth();
  const [randomSound, setRandomSound] = useState<RandomSound>(null);
  ...
  const fetchRandomSound = async (maxSoundId: number) => {
  ...
  const initPlayText = async (sound: string) => {
    if (sound) {
      speech.speak({
        text: sound,
      });
  ...
  if (transcript.toLowerCase()) {
    if (randomSound && transcript.toLowerCase() === randomSound.sound) {
      result = "Correct";
    } else if (randomSound && transcript.toLowerCase() !== randomSound.sound) {
